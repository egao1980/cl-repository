...
(defun make-annotations (spec)
  ...
  (let ((ann (make-hash-table :test 'equal)))
    ...
    ;; Annotation values must be strings — never pass lists through to YASON.
    (setf-when (gethash +ann-description+ ann)
               (let ((d (package-spec-description spec)))
                 (if (stringp d) d (and d (princ-to-string d)))))
    (setf-when (gethash +ann-authors+ ann)
               (let ((a (package-spec-author spec)))
                 (cond ((null a) nil)
                       ((stringp a) a)
                       ((consp a) (format nil "~{~a~^, ~}" a))
                       (t (princ-to-string a)))))
