...
(defun generate-init-file (install-dir config)
  "Generate cl-repo-init.lisp wiring this system's bundled native libs into CFFI.
   ...
   Pushes native/ plus each cffi-library's :search-path onto
   cffi:*foreign-library-directories* so the system's own define-foreign-library
   resolves the bundled .so/.dylib/.dll at load time (native-deps.md, Post-Install).
   Each library's define-foreign-library/canary is recorded in a comment for the
   CFFI canary pattern -- the prebuilt lib then loads transparently once the search
   path is set. CONFIG holds the canonical cffi-libraries alist (NAME . PLIST)."
  (let ((init-path (merge-pathnames "cl-repo-init.lisp" install-dir))
        (libs (config-cffi-libraries config))
        (dir (namestring install-dir)))
